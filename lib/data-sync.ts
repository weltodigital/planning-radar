...
function convertApiApplicationToDb(apiApp: PlanningApiApplication): Omit<PlanningApplication, 'id' | 'location' | 'created_at' | 'updated_at'> {
  return {
    ...
    postcode: apiApp.postcode ? normalizePostcode(apiApp.postcode) : undefined,
    lat: apiApp.lat ? Number(apiApp.lat) : undefined,
    lng: apiApp.lng ? Number(apiApp.lng) : undefined,
    date_validated: apiApp.validated ? parseApiDate(apiApp.validated)?.toISOString().split('T')[0] : undefined,
    date_received: apiApp.received ? parseApiDate(apiApp.received)?.toISOString().split('T')[0] : undefined,
    date_decision: apiApp.decision_date ? parseApiDate(apiApp.decision_date)?.toISOString().split('T')[0] : undefined,
    decision: apiApp.decision || undefined,
    applicant_name: apiApp.applicant || undefined,
    agent_name: apiApp.agent || undefined,
    application_type: apiApp.type || undefined,
    external_link: apiApp.externalLink || undefined,
  }
}
